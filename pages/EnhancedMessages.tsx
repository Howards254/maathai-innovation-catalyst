          ...
          <div className="bg-white rounded-xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="p-4 border-t">
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={async () => {
                    if (!previewFiles.length || !activeConversation) return;
                    setUploading(true);
                    try {
                      for (const file of previewFiles) {
                        const url = await uploadMedia(file, 'messages');
                        await sendMessage(activeConversation, mediaCaption, [url]);
                      }
                      setShowMediaPreview(false);
                      setPreviewFiles([]);
                      setMediaCaption('');
                  ...
                  disabled={uploading}
                  className="px-6 py-2 bg-green-500 text-white rounded-full hover:bg-green-600 disabled:opacity-50 flex items-center gap-2"
                >
                  {uploading ? (
